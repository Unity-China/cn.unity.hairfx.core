...
bReload = True
if(bReload): 
    importlib.reload(TressFX_Exporter)
...
class ProgressBar:
    def __init__ (self,title,steps):
        self.window = cmds.window(title, sizeable=False)
        cmds.columnLayout()

        self.progressControls = []
        self.progressbar = cmds.progressBar(gMainProgressBar, maxValue=steps, width=300, edit=True, isInterruptable=True)
    ...
    def Increment(self):
        cmds.progressBar(self.progressbar, edit=True, step=1)
...
def UI():
    ...
    cmds.checkBox("InvertX", label='Invert X-axis of Hairs', value = False)
...
def SaveTFXBinaryFile(filepath, curves, meshShapedagPath):
    ...
    invertX = cmds.checkBox("InvertX",q = True, v = True)
    ...
    for i in range(numCurves):
        ...
        for j in range(0, numVerticesPerStrand):
            ...
            # X is originally inverted, thus inverted is +, not-inverted is -
            if invertX:
                p.x = pos.x # flip in x-axis
            else:
                p.x = -pos.x
